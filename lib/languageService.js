...
const SNAPSHOT_KEYS = ["instruments", "instrumentation", "instrumentationEvidence", "instrumentEvents", "performance", "arrangement", "genreContextEvidence", "rhythmicGrammar", "productionEvidence", "genreFamily", "primaryGenre", "subgenreCandidates", "confidence", "rhythm", "harmony", "timbre", "texture", "dynamics", "space", "production", "mood", "currentSection", "distinctive", "measurements", "analysisWindow", "moodDimensions", "genreEvidence", "primitives", "detectedIdioms", "impressionConcepts", "verifiedClaims"];
const GROUP_FIELDS = {
...
const languageSchema = {
  type: "object", additionalProperties: false,
  properties: Object.fromEntries(Facets.names.map(category => [category, {
    type: "array", minItems: 0, maxItems: category === "genre" ? 3 : 4, items: descriptorSchema
  }])), required: Facets.names
};
// A routine pool top-up on an already-understood track (reason "pool-low"/"pool-ready") does not
// need the same ceiling as a genuine context change: fewer items per facet, lower reasoning, a
// smaller token budget, and a shorter request timeout. This is the FAST call; everything else
// (a new track, a semantic-epoch change, or a meaningful creative event) is the DEEP call and
// keeps the full schema/effort/budget. Both share the exact same concept-aware schema shape per
// item), so the critic/parser never need to know which mode produced a candidate.
const fastLanguageSchema = {
  type: "object", additionalProperties: false,
  properties: Object.fromEntries(Facets.names.map(category => [category, {
    type: "array", minItems: 0, maxItems: category === "genre" ? 2 : 2, items: descriptorSchema
  }])), required: Facets.names
};
const FAST_REASONS = new Set(["pool-low", "pool-ready"]);
function callMode(reason) { return FAST_REASONS.has(reason) ? "fast" : "deep"; }
const CALL_TUNING = Object.freeze({
  fast: { reasoningEffort: "low", maxOutputTokens: 4500, timeoutMs: 40000, schema: () => fastLanguageSchema },
  deep: { reasoningEffort: "medium", maxOutputTokens: 10000, timeoutMs: 120000, schema: () => languageSchema }
});
...
ASSOCIATION: real aesthetics (Anime, Y2K, Kawaii) can be concise qualified associations with strong evidence; random imaginary scenery/memories cannot. Specific magical-girl aesthetics require independent aestheticEvidence.magicalGirl >= .75, never brightness alone. Empty is valid.
...
Your roles are: (1) specialize a neutral idiom into an established term used by the supported genre/tradition, without changing the underlying primitive or its anchors; (2) compose language from verified claims — never invent a new musical fact because a genre "usually has" it; (3) realize approved context relations in varied wording without changing their meaning; (4) induce aesthetic associations from verified feature clusters.
When evidenceCapsule is present, treat VERIFIED as the only license for concrete musical-fact terms and DISTINCTIVE as the preferred material to verbalize. UNSUPPORTED terms (walking bass, sidechain, 2-step, vocal chop, solo, arpeggio, brush drums, slap bass) must not appear unless they are in VERIFIED. CONTEXT phrases may vary (계열/계보/문법/식 펌핑) but must keep the approved relation target. AESTHETIC may be freer (미학/감성/이미지/질감) and may fuse claims; it must not assert a work, place, franchise, or sample source as fact. Do not propose new historical/cultural relations at runtime.
...
Never infer from a null primitive. Specialization requires primary genre confidence >= .60 and all original anchors. If this is not met, preserve the neutral term.
...
AESTHETIC (association): controlled metaphor and cultural/visual association are allowed, but every term must read as an association, not a claim — keep the existing 미학/연상/계열/감성 qualifier requirement so it never masquerades as fact.
IMPRESSION (mood): concise, genuinely poetic subjective impression is ALLOWED and encouraged here, as long as it synthesizes at least two of the currently-combined musical features (e.g. warmth + rhythm feel, or brightness + dynamics) rather than describing one axis alone. This is the one facet where 향수/낭만/애상-class emotional language belongs.
...
Forbidden everywhere, including IMPRESSION: unobserved scenery/memory/person narratives, song/artist identification, actual recording date/place/creator claims, filling quotas, and the generic AI-poetry vocabulary this project has already rejected as meaningless with any music (과열된 긴장, 냉각된 긴장, 저중력 부유, 유리 같은 고독, 금속성 황홀, and equivalents) — the ban on THOSE specific empty phrases is about genericness, not about poetry itself, so it applies inside IMPRESSION too. Arbitrary invented *core/코어 labels are forbidden; only terms registered as established scene genres are accepted.
...
Example A — Jazz specialization:
Input: polyphonic texture, voiceCount 4, dispersion .72, brass/reed active, subdivision 1.62, bassFunction walking, Jazz .71.
Output includes 집단 즉흥, 혼 섹션, 스윙 필, 워킹 베이스; it excludes 브라스 솔로 because solo evidence is absent.
Example B — dance specialization:
Input: accentPlacement .06, accentPeriodicity 4, pulseRegularity .88, periodicDucking .74, synth active, House .79.
Output includes 4/4 플로어, 사이드체인 펌핑, 하우스 계열; it excludes 드롭 without buildup/release evidence.
...
Use concise Korean or established English musical terms, normally 1–4 words. Resolve contradictions and duplicates. Return only schema JSON, no reasoning.`;
...
function validateLanguageInput(body = {}) {
    ...
    snapshot.verifiedClaims = {
      items: (Array.isArray(source.verifiedClaims.items) ? source.verifiedClaims.items : []).slice(0, 24)
        .filter(item => item && typeof item.concept === "string").map(item => ({
          ...
          confidence: Math.min(1, Math.max(0, Number(item.confidence) || 0)),
          evidence: (Array.isArray(item.evidence) ? item.evidence : []).filter(path => typeof path === "string").slice(0, 6)
        })),
  ...
  if (JSON.stringify(snapshot).length > 24000) throw httpError("Semantic snapshot is too large.", 400, "snapshot_too_large");
...
function createLanguageRequest(input, options = {}) {
  ...
  return {
    ...
    store: false,
    prompt_cache_key: "music-shower-language-v6",
    prompt_cache_options: { mode: "explicit", ttl: "30m" },
    input: [
      { role: "developer", content: [{ type: "input_text", text: languageInstructions, prompt_cache_breakpoint: { mode: "explicit" } }] },
...
function parseLanguageResponse(response) {
  ...
  for (const category of Facets.names) {
