...
function createUsageLedger({ file = DEFAULT_LEDGER, now = Date.now } = {}) {
  ...
  async function record(purpose, api, request, call) {
    const startedAt = now();
    try {
      const response = await call();
      write({ at: new Date(startedAt).toISOString(), t: startedAt, purpose, api, model: request?.model || "",
        ok: true, latencyMs: now() - startedAt, finishReason: response?.choices?.[0]?.finish_reason || response?.status || null,
        ...usageFields(response?.usage) });
      return response;
    } catch (error) {
      write({ at: new Date(startedAt).toISOString(), t: startedAt, purpose, api, model: request?.model || "",
        ok: false, latencyMs: now() - startedAt, error: String(error?.message || error).slice(0, 160),
        ...usageFields(error?.usage) });
      throw error;
    }
  }

  // A client with the same call surface the callers already use, tagged with a purpose.
  function clientFor(client, purpose) {
    if (!client) return null;
    return {
      chat: { completions: { create: (request, options) =>
        record(purpose, "chat.completions", request, () => client.chat.completions.create(request, options)) } },
      responses: { create: (request, options) =>
        record(purpose, "responses", request, () => client.responses.create(request, options)) }
    };
  }
