...
function calibrateStructuredPacket(packet, continuity = {}) {
  ...
  const independentCaps = {
    audibleObservations: 0.72, signatureRelations: 0.70, genreHypotheses: 0.76,
    contextHypotheses: 0.68, aestheticConcepts: 0.72, impressions: 0.72
  };
  const assistedCaps = {
    audibleObservations: 0.58, signatureRelations: 0.56, genreHypotheses: 0.60,
...
function toObservations(review = {}, options = {}) {
  ...
  if (structured) {
    ...
    for (const obs of structured.audibleObservations || []) {
      ...
      const requested = String(obs?.category || "").toLowerCase();
      const category = { instrument: "instrumentation", instruments: "instrumentation", tempo: "rhythm" }[requested] || requested;
      const conf = obs?.confidence ?? 0.65;
      if (text && ["rhythm", "instrumentation", "performance", "arrangement", "production", "dynamics"].includes(category)) {
