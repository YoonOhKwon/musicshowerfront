...
const CULTURAL_LABELS = {
  ...
  y2k: 'Y2K 미학 연상', vaporwave: '베이퍼웨이브 미학 연상', scene: '씬 연상', culture: '문화 연상', artist: '스타일 연상'
};
const LABELS_BY_TYPE = { 'musical-claim': MUSICAL_LABELS, 'cultural-or-historical': CULTURAL_LABELS };
...
// produce an observation; everything else (including every "impression"-type claim, which by
// definition matched neither MUSICAL nor CULTURAL and so has no keyword to anchor a label to) is
// honestly dropped rather than guessed -- it stays visible in reviewCaption()'s own report, just
// not promoted into structured evidence.
