...
class RealtimeMusicSession {
  ...
  reset(sampleRate, preservePool = false) {
    ...
    if (!preservePool) {
      ...
      this.association = { cues: new Map(), items: [], tier: null, primary: null, key: "", inFlight: false,
  ...
  requestAssociation() {
    ...
    const { tier, primary } = Grounded.resolutionTier(this.state);
    ...
    const evidence = Grounded.buildEvidence({ state: this.state, concepts, cues });
    const key = JSON.stringify([tier, primary?.label, evidence.map(item => item.text)]);
    if (key === association.key) return;
    Object.assign(association, { key, inFlight: true, tier, primary });
    const controller = new AbortController();
    this.controllers.add(controller);
  ...
  associationSnapshot() {
    ...
    if (association.calls > 0 && !association.inFlight) {
      const { tier } = Grounded.resolutionTier(this.state);
  ...
  requestEnglish(tokens) {
    ...
    const items = tokens.filter(token => !token.textEn).map(({ text, layer }) => ({ text, layer }));
