const GenreContext = (() => {
  ...
  class Engine {
    constructor(data = {}, aestheticAxisEngine = null) {
      this.data = data; this.aestheticAxisEngine = aestheticAxisEngine;
    }
    evaluate(state = {}) {
    ...
    // Section 4 used to live here: a hand-authored table (data/genreCompositions.json) that named
    // "Future Funk 계열" and "French House 계열" when the classifier's top-K matched a written-down
    // label combination. It has been removed along with every other hardcoded genre-name rule.
    // The classifier's 400 classes really are a coordinate system rather than a wall -- but the
    // name for a point outside them comes from a listener, not from a table someone maintained.
    // genreHypothesisEngine now derives composites from Music Flamingo's open-vocabulary naming
    // corroborated against that same top-K, so any composite is reachable, not a chosen six.
