const InstrumentationEvents = (() => {
  ...
  const labels = {
    ...
    bass: "베이스", electricbass: "일렉트릭 베이스", synthesizer: "신스", synth: "신스",
    piano: "피아노", electricpiano: "일렉트릭 피아노", acousticpiano: "어쿠스틱 피아노",
    electricguitar: "일렉트릭 기타", acousticguitar: "어쿠스틱 기타", guitar: "기타",
    strings: "스트링", orchestra: "오케스트라", saxophone: "색소폰", trumpet: "트럼펫",
    brass: "브라스", flute: "플루트", organ: "오르간", cello: "첼로", violin: "바이올린",
  ...
  const FAMILIES = Object.freeze({
    ...
    plucked: ["guitar", "acousticguitar", "electricguitar", "banjo", "harp", "koto", "sitar"],
    keys: ["piano", "electricpiano", "acousticpiano", "organ", "harpsichord", "accordion"],
    synth: ["synthesizer", "synthpad", "synthlead"], bass: ["bass", "electricbass", "doublebass"],
    percussion: ["drums", "percussion", "timpani"], voice: ["voice", "vocal", "choir"]
  });
  ...
  function performance(observed, frames, f) {
    ...
    const supportedSolo = top && frames.length >= 5 && top.confidence >= 0.8 && top.confidence - second >= 0.3 &&
    ...
    // Walking bass requires bass-specific pitch and pulse observations, not Jazz + bass.
    const bass = observed.find(x => /bass/.test(x.id));
    const walking = bass?.confidence >= 0.65 && Number.isFinite(f.bassPitchMotion) && Number.isFinite(f.bassOnsetRegularity)
      ? Math.min(bass.confidence, f.bassPitchMotion, f.bassOnsetRegularity) : null;
    ...
    else if (walking >= 0.8) bassFunction = "walking";
  ...
  class Engine {
    ...
    reset() { this.frames = []; this.events = []; this.lastAt = -Infinity; this.current = null; }
    update(instruments = [], features = {}, at = Date.now()) {
      ...
      const perf = performance(observed, history, features);
      const total = observed.reduce((sum, x) => sum + x.confidence, 0);
      observed.forEach(x => { x.dominance = x.confidence / Math.max(0.001, total); });
      const families = familyRollup(observed);
      const dispersion = dominanceDispersion(observed);
      const leadIds = [...history.map(frame => frame.observed[0]?.id), observed[0]?.id].filter(Boolean);
      ...
      for (const id of previousIds) {
        ...
        const current = observed.find(item => item.id === id);
      ...
      if (perf.leadLikelihood >= 0.8 && perf.dominanceChange > 0.2 && history.length >= 4 && top) addEvent(top, "신스 리드 유입", "lead", perf.leadLikelihood);
      this.events = this.events.filter(event => at - event.at <= this.eventTtlMs).slice(-12);
      this.frames = [...history, { at, observed, density: Number.isFinite(features.accompanimentDensity) ? features.accompanimentDensity : 0.5 }].slice(-this.capacity);
      ...
      this.current = { instrumentation: { observed, families, dominanceDispersion: dispersion, leadTransitionRate,
          confidence: top?.confidence || 0, resolution: "model classes; no source separation" },
