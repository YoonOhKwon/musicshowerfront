...
async function main() {
  ...
  socket.on("message", async data => {
    ...
    if (message.type === "ready") socket.send(JSON.stringify({ type: "start", sampleRate: 16000, channels: 1, format: "f32le" }));
    ...
    const flamingo = message.tokens.filter(item => item.sourceFamily === "directAudio" && /[가-힣]/.test(item.text));
    if (gotPcmFeatures && gotFinalPool && flamingo.length && message.analysis.status === "listening") {
      console.log(JSON.stringify({ ok: true, poolSource: message.poolSource,
